...
/*
Name: Emmanuel Giron
Description: Edit an existing tool in the Rocket Rentals inventory (UPDATE operation).
Loads tool data via query string ?id=X using a prepared statement,
validates input, and updates the row.
*/

require_once __DIR__ . '/../includes/db.php';

// Validate the query string ID
$toolId = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
...
// Fetch tool types for the dropdown
$typeStmt = $pdo->query(
...
// Load the existing tool using a prepared statement
$loadStmt = $pdo->prepare("SELECT * FROM tools WHERE id = :id");
$loadStmt->execute([':id' => $toolId]);
$tool = $loadStmt->fetch();
...
$values = [
   ...
   'delivery_only' => (string) $tool['delivery_only'],
   'type_id'       => (string) $tool['type_id'],
   'image_url'     => $tool['image_url'] ?? '',
   'is_active'     => (string) $tool['is_active'],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   if (empty($errors['slug'])) {
      $slugCheck = $pdo->prepare("SELECT COUNT(*) FROM tools WHERE slug = :slug AND id != :id");
      $slugCheck->execute([':slug' => $values['slug'], ':id' => $toolId]);
      if ($slugCheck->fetchColumn() > 0) {
         $errors['slug'] = 'This slug is already in use by another tool.';
      }
   }
   ...
   if (empty($errors)) {
      $updateStmt = $pdo->prepare(
         "UPDATE tools
          SET type_id = :type_id, name = :name, slug = :slug, description = :description,
              daily_rate = :daily_rate, deposit = :deposit, delivery_only = :delivery_only,
...
<head>
   ...
   <title>Edit Tool | Rocket Rentals Admin</title>
   <style>
...
<body>
   ...
   <h1>Edit Tool: <?php echo htmlspecialchars($values['name']); ?></h1>

   <div class="form-card">
      <form method="POST" action="edit-tool.php?id=<?php echo (int) $toolId; ?>">
         <div class="field">
         ...
         <div class="field">
            ...
            <input type="text" id="slug" name="slug" value="<?php echo htmlspecialchars($values['slug']); ?>" required>
         ...
         <div class="field">
            <div class="checkbox-field">
               <input type="checkbox" id="is_active" name="is_active" value="1"
                      <?php echo $values['is_active'] === '1' ? 'checked' : ''; ?>>
               <label for="is_active">Active (visible on customer site)</label>
            </div>
         </div>
